...
} elseif ($_GET['code'] || ($_POST['password_reset_code'] && (!trim($_POST['password']) || !trim($_POST['confirm_password']) || ($_POST['password'] != $_POST['confirm_password'])))) {
...
<form class="form-user" method="post" action="/user/forgot-password"><h2><?php echo $menus['main']['logo']['name']; ?></h2>
	...
	<?php if ($_POST && $_POST['password'] != $_POST['confirm_password']) {
		echo '<div class="form-user alert alert-danger">Password mismatch or empty.</div>';
	}?>
...
<?php
} elseif (trim($_POST['password']) && $_POST['password'] == $_POST['confirm_password']) {
...
} elseif (($types['webapp']['user_theme'] ?? false) && file_exists(THEME_PATH . '/user-forgot-password.php')) {
